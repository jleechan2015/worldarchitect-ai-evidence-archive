...
## Test Mode
- **TESTING env var:** None (not applicable — this suite uses `TESTING_AUTH_BYPASS=true` for auth bypass; `TESTING` is a separate env var used by some unrelated test harnesses)
- **MOCK_SERVICES_MODE env var:** false
- **Mode:** Real API calls via MCP HTTP JSON-RPC

## Execution Environment
- Server running at port 59367
- Process: <redacted-home-path> -m gunicorn mvp_site.main:app --bind 0.0.0.0:59367 --workers 1 --worker-class gthread --threads 4 --timeout 600 --max-requests 0 --access-logfile - --error-logfile - --log-level info
