# PR #7022 Original Bug Repro Criteria - Awaiting Human Sign-off
...
Future `/repro` evidence for PR #7022 must reference this file and fill out the verdict table below before claiming success.
...
## Red/green code provenance

A valid fresh RED replay must not run on the PR #7022 fixed codepath. Acceptable red provenance is one of:

- A pre-fix checkout, such as the merge-base or explicitly identified commit before the location/stale-context fix.
- A pre-fix deployment whose deployed SHA/config is known and recorded.
- The historical original source campaign artifact, clearly labeled `HISTORICAL RED ARTIFACT` rather than a fresh red replay.

A remote dev app replay is not valid red evidence unless its deployed SHA/config is proven pre-fix. If the deployed version is unknown or may include the PR fix, classify the attempt as `AMBIGUOUS ENVIRONMENT`, `RELATED`, or `NON-REPRO`; do not call it RED.

A valid GREEN replay must run against the candidate PR #7022 fixed codepath and the same approved scenario, then show no visible repeated old narrative.

## Approved red environment

Jeffrey approved the remote dev app as acceptable RED provenance for this repro lane because it is origin-main, not the PR fixed branch:

```text
https://mvp-site-app-dev-i6xf2p72ka-uc.a.run.app/
```

For this bug, fresh red repro attempts may use that remote dev app, provided the attempt still satisfies the same-symptom criteria above. The evidence must state that this environment was human-approved as the origin-main red lane.
